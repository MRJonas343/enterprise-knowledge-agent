...
This repository builds an Enterprise Knowledge Agent: grounded enterprise answers over governed knowledge, using Microsoft Foundry, Foundry IQ and Azure services. **Phases 0-2 are accepted: the infrastructure, the knowledge corpus, the synchronization path and cited retrieval all work end to end. The Phase 2 retrieval acceptance record is in [`docs/verification/phase-2-retrieval-acceptance.md`](docs/verification/phase-2-retrieval-acceptance.md). The active boundary is the rest of the MVP: FastAPI and the Next.js frontend.**
...
## Current Boundary: Phase 3-5, the MVP Path

Phases 0 to 2 are accepted. The Phase 2 retrieval gate was cleared on 2026-09-19; the evidence is in [`docs/verification/phase-2-retrieval-acceptance.md`](docs/verification/phase-2-retrieval-acceptance.md).

The active boundary is the rest of the MVP path:

- Phase 3: Foundry Agent Service integrated with the knowledge base. Deployed and answering with citations; formal acceptance still pending.
...
- Foundry Agent Service, FastAPI, Next.js, MCP, OpenTelemetry, evaluations, security hardening, and SharePoint are later boundaries described in the roadmap, not current implementation targets.
...
Current status: Phases 0-2 accepted on 2026-09-19 and Phase 3 deployed. The next owner should record formal Phase 3 acceptance, then build the FastAPI gateway (Phase 4) and the Next.js frontend (Phase 5) to reach the MVP gate. Everything the knowledge base and the agent need already exists: `enterprise-knowledge/` is the source corpus, `infra/terraform/` owns the Azure control plane, and `src/scripts/` owns the data-plane objects. Evaluations, observability, security hardening and SharePoint remain behind the MVP gate.
